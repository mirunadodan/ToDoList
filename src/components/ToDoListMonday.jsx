...
const initialToDos = () => {
  const data = JSON.parse(localStorage.getItem("todosMonday"));
  if (!data) return []
  else return data
  console.log(initial)
}
  
export default function ToDoListMonday({day, color}) {
  const [toDos, setToDos] = useState(initialToDos)

  useEffect(() => {
    localStorage.setItem("todosMonday", JSON.stringify(toDos))
  }, [toDos])
